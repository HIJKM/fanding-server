...
router.post('/', (0, validation_1.validateNonEmptyBodyMiddleware)(), async (req, res) => {
    try {
        ...
        // Create poll
        const poll = await governanceService_1.governanceService.createPoll({
...
router.get('/:musicianId', async (req, res) => {
    try {
        ...
        if (limit > 100)
            limit = 100;
        const { polls, total } = await governanceService_1.governanceService.getMusicianPolls(musicianId, skip, limit);
 ...
 * }
 */
router.post('/:pollId/vote', (0, validation_1.validateNonEmptyBodyMiddleware)(), async (req, res) => {
    try {
        ...
        // Cast vote
        const poll = await governanceService_1.governanceService.vote({
            pollId,
...
router.get('/:pollId/results', async (req, res) => {
    try {
        const { pollId } = req.params;
        const results = await governanceService_1.governanceService.getPollResults(pollId);
