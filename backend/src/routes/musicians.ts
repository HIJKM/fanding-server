 ...
 * }
 */
router.post('/signup', validateNonEmptyBodyMiddleware(), async (req: Request, res: Response) => {
  try {
    const { userId, musicianName, genre, bio, profileImage, walletAddress, socialLinks } =
      req.body;
    ...
    // Create musician
    const musician = await MusicianService.createMusician({
      userId,
 ...
 * }
 */
router.post('/:id/deploy-token', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    // Get musician
    const musician = await MusicianService.getMusicianById(id);
    ...
    if (musician.isTokenDeployed) {
      ...
      return res.status(409).json(
        formatError(
          'Musician already has a deployed token',
          409,
          req.originalUrl
        )
      );
    ...
    if (walletAddress && walletAddress !== musician.walletAddress) {
      ...
      return res.status(400).json(
        formatError('Wallet address does not match musician profile', 400, req.originalUrl)
      );
    }

    // Check network availability
    console.log('   🌐 네트워크 가용성 확인 중...');
    const networkAvailable = await TokenDeploymentService.isNetworkAvailable();
    if (!networkAvailable) {
    ...
    if (result.status === 'failed') {
      ...
      return res.status(500).json(
        formatError(`Token deployment failed: ${result.errorMessage}`, 500, req.originalUrl)
      );
    }
    ...
    const updated = await MusicianService.updateTokenInfo(
      id,
      result.tokenAddress,
      result.deploymentTxHash
    );
  ...
  try {
    ...
    if (!musician.isTokenDeployed || !musician.tokenAddress) {
      return res.status(404).json(
        formatError('Musician has not deployed a token yet', 404, req.originalUrl)
      );
    }
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await MusicianService.listMusicians(skip, limit);

    // Transform response to map _id to musicianId
    const transformedMusicians = result.musicians.map((musician: any) => ({
      musicianId: musician._id,
      musicianName: musician.musicianName,
      genre: musician.genre,
      walletAddress: musician.walletAddress,
    ...
    return res.status(500).json(formatError(error, 500, req.originalUrl));
  }
});
...
router.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    if (!query || query.trim().length === 0) {
      return res.status(400).json(
        formatError('Search query is required', 400, req.originalUrl)
      );
    }
    ...
    const result = await MusicianService.searchByName(query, skip, limit);
