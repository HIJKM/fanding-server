...
function validateAddressMiddleware() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const validation = WalletValidator.validateApiAddress(address);
    if (!validation.valid) {
...
function validateAndGenerateSymbolMiddleware() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const symbol = SymbolGenerator.generateSymbol(name, false);
 ...
 */
function validateQueryParamsMiddleware(expectedParams: { [key: string]: string }) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [param, expectedType] of Object.entries(expectedParams)) {
      ...
      if (expectedType === 'address' && value) {
        const validation = WalletValidator.validateApiAddress(String(value));
        if (!validation.valid) {
...
export {
  ...
  validateMusicianNameMiddleware,
  validateAndGenerateSymbolMiddleware,
  validateNonEmptyBodyMiddleware,
