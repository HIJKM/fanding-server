...
class SymbolGenerator {
   ...
   */
  private static readonly KOREAN_TO_LATIN: { [key: string]: string } = {
    // Consonants (자음)
    ㄱ: 'G',
    ㄲ: 'GG',
    ㄴ: 'N',
   ...
   */
  private static isKorean(char: string): boolean {
   ...
   */
  private static decomposeKorean(char: string): string[] {
    const code = char.charCodeAt(0);
   ...
   */
  private static romanize(koreanText: string): string {
    let result = '';
   ...
   */
  private static extractInitials(romanized: string): string {
   ...
   */
  static generateSymbol(musicianName: string, ensureUniqueness: boolean = true): string {
   ...
   */
  static generateSymbolCandidates(musicianName: string, count: number = 3): string[] {
